...
export default function Home() {
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      setStudentName('');
      await fetchNamesList(); // Re-fetch names to update the list
    } catch (e) {
        ...
        <Accordion type="single" collapsible className="w-full rounded-lg border bg-card text-card-foreground shadow-md">
          <AccordionItem value="item-1" className="border-b-0">
            ...
            <AccordionContent className="px-6 pt-0 pb-4">
              <div className="max-h-[calc(70vh)] overflow-y-auto p-2 rounded-md border border-input bg-background">
                {isLoadingNames ? (
                  ...
                  <ul className="divide-y divide-border/30">
                    {fetchedNames.map((name, index) => (
                      <li key={index} className="py-2 px-2 text-sm text-foreground/90">
                        {name}
                      </li>
                    ))}
                  </ul>
